...
import { dedup, getActiveWishlist, normalizeWishlist, normalizeProduct, getCustomerId } from '../../../utils'
...
const getWishlist: WishlistEndpoint['handlers']['getWishlist'] = async ({
  ...
  const products = (await config.sdkFetch<ClientResponse<ProductPagedQueryResponse>>({
    query: 'products',
    method: 'get',
    variables: {
      where: `id in (${dedup(activeWishlist.lineItems!.map(lineItem => `"${lineItem.productId}"`)).join(", ")})`
    }
  })).body?.results;
